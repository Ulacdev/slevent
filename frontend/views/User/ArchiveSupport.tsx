...
export const ArchiveSupport: React.FC = () => {
    ...
    return (
        <div className="pb-16 space-y-6 max-w-7xl mx-auto">
            ...
            <div className="bg-transparent border border-[#2E2E2F]/10 dark:border-white/10 rounded-xl overflow-hidden">
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                        <thead className="bg-[#2E2E2F]/5 dark:bg-white/5 border-b border-[#2E2E2F]/5 dark:border-white/5">
                            <tr>
                                ...
                                <th className="px-8 py-5 text-[10px] font-black uppercase tracking-[0.2em] text-[#2E2E2F] dark:text-white text-right">Date Archived</th>
                            </tr>
                        </thead>
                            ...
                            {archivedTickets.length === 0 ? (
                                <tr>
                                    <td colSpan={3} className="px-8 py-20 text-center">
                                        <p className="text-sm font-bold text-[#2E2E2F] dark:text-white uppercase tracking-widest">No archived tickets found</p>
                                    </td>
                                </tr>
                            ) : (
                                archivedTickets.map((t) => (
                                    <tr key={t.notification_id} className="hover:bg-[#2E2E2F]/5 dark:hover:bg-white/5 transition-colors cursor-pointer" onClick={() => openThread(t)}>
                                        <td className="px-8 py-6">
                                            <p className="text-sm font-bold text-[#2E2E2F] dark:text-white">{t.title}</p>
                                            <p className="text-xs text-[#2E2E2F] dark:text-white/60 truncate max-w-md">{t.message}</p>
                                        </td>
                                        <td className="px-8 py-6">
            ...
            {selectedTicket && (
                <div className="fixed inset-0 bg-black/40 backdrop-blur-md z-[9999] flex items-center justify-center p-4 sm:p-6 transition-opacity animate-in fade-in duration-300" onClick={() => setSelectedTicket(null)}>
                    <div className="relative w-full max-w-2xl max-h-full bg-[#F2F2F2] dark:bg-[#111111] shadow-2xl rounded-xl border border-[#2E2E2F]/10 dark:border-white/10 overflow-hidden flex flex-col h-[90vh]" onClick={e => e.stopPropagation()} style={{ zoom: 0.85 }}>
                        {/* Header */}
                        <div className="px-8 py-5 border-b border-[#2E2E2F]/5 dark:border-white/5 flex items-center justify-between bg-transparent sticky top-0 z-10">
                            <button onClick={() => setSelectedTicket(null)} className="p-3 hover:bg-[#2E2E2F]/5 dark:bg-white/5 rounded-full text-[#2E2E2F] dark:text-white transition-all">
                            ...
                            <button 
                                ...
                                className="p-3 hover:bg-[#2E2E2F]/5 dark:bg-white/5 rounded-full text-[#2E2E2F] dark:text-white transition-all hover:text-[#38BDF2]"
