...
export const EventDiscoveryPage: React.FC = () => {
    ...
    return (
        <div className="flex min-h-screen bg-[#F2F2F2]">
            ...
            {/* Mobile Filter FAB Toggle */}
            <button
                onClick={() => setIsMobileFilterOpen(true)}
                className="md:hidden fixed bottom-6 right-6 z-30 bg-[#38BDF2] text-white p-4 rounded-full shadow-lg active:scale-95 transition-transform"
            ...
            {isMobileFilterOpen && (
                <div className="md:hidden fixed inset-0 z-50">
                    ...
                    />
                    <div className="absolute bottom-0 left-0 right-0 max-h-[85vh] bg-[#F9FAFB] rounded-t-3xl overflow-hidden animate-in slide-in-from-bottom duration-300">
                        <div className="flex items-center justify-between p-4 border-b border-black/5">
                            <h2 className="text-lg font-black text-black uppercase">Filters</h2>
                            <button 
                                onClick={() => setIsMobileFilterOpen(false)}
                        ...
                        <div className="p-4 overflow-y-auto max-h-[70vh] space-y-6">
                                    ...
                                    {categories.map(cat => {
                                        ...
                                        return (
                                            <button
                                                key={cat.key}
                                                onClick={() => toggleCategory(cat.key)}
                                                className={`p-3 rounded-xl text-xs font-bold transition-all ${isChecked ? 'bg-[#38BDF2] text-white' : 'bg-[#F2F2F2] text-black'}`}
                                            >
                                                {cat.label}
                                            </button>
                                        );
                            ...
                            {/* Mobile Date Filters */}
                            <div className="space-y-3">
                                <h3 className="text-xs font-black uppercase tracking-[0.1em] text-black">When</h3>
                                <div className="grid grid-cols-2 gap-2">
                                    {[
                                        ...
                                        <button
                                            key={date.id}
                                            onClick={() => setSelectedDate(date.id)}
                                            className={`p-3 rounded-xl text-xs font-bold transition-all ${selectedDate === date.id ? 'bg-[#38BDF2] text-white' : 'bg-[#F2F2F2] text-black'}`}
                                        >
                                            {date.label}
                                        </button>
                                    ))}
                            ...
                            <div className="space-y-3">
                                <h3 className="text-xs font-black uppercase tracking-[0.1em] text-black">Price</h3>
                                <div className="grid grid-cols-3 gap-2">
                                    {[
                                        { id: 'all', label: 'All' },
                                        { id: 'free', label: 'Free' },
                                        ...
                                        <button
                                            key={price.id}
                                            onClick={() => setSelectedPrice(price.id)}
                                            className={`p-3 rounded-xl text-xs font-bold transition-all ${selectedPrice === price.id ? 'bg-[#38BDF2] text-white' : 'bg-[#F2F2F2] text-black'}`}
                                        >
                                            {price.label}
                                        </button>
                                    ))}
                            ...
                            <button
                                onClick={() => setIsMobileFilterOpen(false)}
                                className="w-full py-4 bg-black text-white rounded-xl font-black uppercase text-sm"
                            >
                                Apply Filters
                            </button>
                        </div>
